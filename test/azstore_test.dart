...
Future<void> testFilterTable() async {
  ...
  final result=await storage.filterTableRows(tableName: 'profiles',
...
Future<void> testUploadTableNode() async {
  final storage = AzureStorage.parse(connectionString);
  try {
    const myPartitionKey='sfsdfsrg57865746';
    const myRowKey='237';
    final rowMap=<String, dynamic>{'Address':'Santa Clara',
      'Age':23,
      'AmountDue':200.23,
...
Future<void> testGetTableRow() async {
  final storage = AzureStorage.parse(connectionString);
  try {
    const myPartitionKey='fgtdssdas';
    const myRowKey='232';
    final result=await storage.getTableRow(
        tableName: 'profiles',
        partitionKey:myPartitionKey,
        rowKey:myRowKey,
...
Future<void> testDeleteBlob() async {
  final storage = AzureStorage.parse('your connection string');
  try {
...
Future<void> testGetQData() async {
  ...
  try {
    const qName='myqueue';
    final result = await storage.getQData(qName);
...
Future<void> testGetQList() async {
  final storage = AzureStorage.parse(connectionString);
  final result=await storage.getQList();
...
Future<void> testGetQMessages() async {
  ...
  print('working on results...');
  try {
    final result = await storage.getQmessages(qName: 'ttable',//Required
...
Future<void> testPeekQMessages() async {
  final storage = AzureStorage.parse(connectionString);
  try {
    final result = await storage.peekQmessages(qName: 'ttable');
